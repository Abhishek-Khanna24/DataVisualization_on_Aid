...
function legend2({
  ...
  size_tick = 6,
  width = 250, 
  height = 100 + size_tick,
  marginTop = 0,
  marginRight = 0,
  marginBottom = -20 + size_tick,
  marginLeft = 0,
  ticks = width / 64,
  tickFormat,
  val_tick
 ...
 if (color.interpolator) {
  ...
  if (!x.ticks) {
    if (val_tick === undefined) {
      const n = Math.round(ticks + 1);
      val_tick = d3.range(n).map(i => d3.quantile(color.domain(), i / (n - 1)));
    }
...
// margin convention
const margin = {top: 9, right: 1, bottom: 1, left: 1};
const visWidth = 899 - margin.left - margin.right;
const visHeight = 399 - margin.top - margin.bottom;
